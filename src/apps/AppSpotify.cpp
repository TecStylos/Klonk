	...
	: Application(fb)
{
	...
	uiTrackName->setCbOnUpdate(
		[](UIElement* pElem, void* pData)
		{
			...
			if (pElem->width() > app.m_uiRoot.width())
			{
				...
				if (pElem->posX() > 16 && scrollDirection != -1)
				{
					scrollDirection = -1;
					lastScrollChange = now;
				}
				else if (pElem->posX() + pElem->width() < app.m_uiRoot.width() - 16 && scrollDirection != 1)
				{
					scrollDirection = 1;
					lastScrollChange = now;
				}

				if (now - lastScrollChange > scrollDelay)
				{
	...
	uiSeekbar->setCbOnDown(
		[](UIElement* pElem, int x, int y, void* pData)
		{
			...
			int x = (x - pElem->posX()) * app.m_trackLen / pElem->width();
